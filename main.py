...
max_k = n if n >= 2 else 2
default_k = n // 2 if n // 2 >= 1 else 1
k = st.sidebar.slider("Quórum mínimo (k)", 1, max_k, default_k)
...
ponto_atual = pd.DataFrame({"p": [p], "Disponibilidade": [analitico]})

ponto = (
    alt.Chart(ponto_atual)
    .mark_point(size=100)
    .encode(x="p:Q", y="Disponibilidade:Q", tooltip=["p", "Disponibilidade"])
)
...
p_n = col_n1.number_input(
    "Probabilidade de servidor ativo (p)",
    min_value=0.001,
    max_value=1.0,
    value=0.9,
    step=0.001,
...
if n_calculado is None:
    st.error(
        "Não foi possível encontrar um N viável (até 10.000 servidores). Tente reduzir a disponibilidade alvo ou aumentar p."
    )
else:
    disp_real = disponibilidade_analitica(n_calculado, int(k_n), p_n)
    st.success(
        f"Para p = {p_n}, k = {int(k_n)} e disponibilidade ≥ {A_alvo:.9g}:  **N mínimo = {n_calculado}** (disponibilidade real: {disp_real:.9f})"
    )

st.subheader("Tabela padrão: p = 0.5, k = 1")
st.markdown(
    "Número mínimo de servidores necessário para atingir cada nível de disponibilidade com p = 0.5 e k = 1."
)
